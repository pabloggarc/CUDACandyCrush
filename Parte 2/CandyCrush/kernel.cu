...
void hardWare(int N, int M) {
    ...
    //Obtener el numero maximo de hilos por bloque y por grid
    int hilos_maximos_bloque = deviceProp.maxThreadsDim[1];
...
int main(int argc, char* argv[]) {
    ...
    while (vidas > 0) {
        ...
        dim3 dimBlock(bloque_fila, bloque_col);
        dim3 dimGrid(grid_fila, grid_col);
